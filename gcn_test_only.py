...
print("DEBUG: gcnmain.py - after import gcnmodel")
sys.stdout.flush()

logging.basicConfig(format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p', level=logging.INFO)
print("DEBUG: gcnmain.py - after logging config")
sys.stdout.flush()
logging.info('In order to work for big datasets fix https://github.com/Theano/Theano/pull/5721 should be applied to theano.')
print("DEBUG: gcnmain.py - after logging info")
sys.stdout.flush()
np.random.seed(77)
print("DEBUG: gcnmain.py - after seeded")
sys.stdout.flush()
model_args = None
...
def preprocess_data(data_home, **kwargs):
    print(f"DEBUG: preprocess_data called with {data_home}")
    sys.stdout.flush()
    ...
    print("DEBUG: dl.load_data() starting...")
    sys.stdout.flush()
    dl.load_data()
    print("DEBUG: dl.load_data() finished. dl.assignClasses() starting...")
    sys.stdout.flush()
    dl.assignClasses()
    print("DEBUG: dl.assignClasses() finished. dl.tfidf() starting...")
    sys.stdout.flush()
    dl.tfidf()
    print("DEBUG: dl.tfidf() finished.")
    sys.stdout.flush()
    ...
    print("DEBUG: dl.get_graph() starting...")
    sys.stdout.flush()
    dl.get_graph()  
    print("DEBUG: dl.get_graph() finished.")
    sys.stdout.flush()
...
def main(data, args, **kwargs):
    ...
    print("DEBUG: Initializing GraphConv...")
    sys.stdout.flush()
    ...
    print("DEBUG: GraphConv initialized. Building model...")
    sys.stdout.flush()
    clf.build_model(A, use_text=args.notxt, use_labels=args.lp, seed=model_args.seed)
    print("DEBUG: Model built.")
    sys.stdout.flush()

    print(f"DEBUG: fractions = {fractions}")
    sys.stdout.flush()
    for percentile in fractions:
        print(f"DEBUG: Inside loop, percentile={percentile}")
        sys.stdout.flush()
        ...
        if args.load:
            ...
            if clf.fitted:
                clf.reset()
            print("DEBUG: Fitting model...")
            sys.stdout.flush()
# [SKIPPED by Test Mode]             clf.fit(X, A, Y, train_indices=train_indices, val_indices=dev_indices, n_epochs=10000, batch_size=batch_size, max_down=args.maxdown, verbose=verbose, seed=model_args.seed)

            # --- Test Mode Block Start ---
            import pickle
            import gzip
            import lasagne
            ...
            print("[Test Mode] Injecting weights into 'l_out'...")
            if 'clf' in locals():
                model_instance = locals()['clf']
            elif 'model' in locals():
                model_instance = locals()['model']
            else:
                print("Error: 모델 객체(clf)를 찾을 수 없습니다.")
                exit()

            # [수술 진행] l_out에 가중치 주입
            try:
                if hasattr(model_instance, 'l_out'):
                    lasagne.layers.set_all_param_values(model_instance.l_out, saved_weights)
                    print("✅ Weights injected successfully into 'l_out'!")
                else:
                    print("❌ Error: 'l_out' 속성이 없습니다.")
                    exit()
            except Exception as e:
                print(f"Error injecting weights: {e}")
                exit()

            print("[Test Mode] Preparing Test Data...")
            # 1. Test Index 찾기
            target_idx = locals().get('test_indices')
            if target_idx is None:
                for name in ['test_idx', 'idx_test', 'test_ids', 'ids_test']:
                    if name in locals():
                        target_idx = locals()[name]
                        break

            # 2. 정답(Label) 추출
            if 'Y' in locals():
                full_Y = locals()['Y']
                target_label = full_Y[target_idx]
                if len(target_label.shape) > 1:
                    target_label = np.argmax(target_label, axis=1)
            else:
                print("⚠️ Warning: 정답 변수 'Y'를 못 찾았습니다.")
                target_label = None

            # 3. [핵심] X와 A(Graph) 찾기 (여러 이름 시도)
            input_X = None
            for name in ['X', 'features', 'x_data', 'inputs']:
                if name in locals():
                    input_X = locals()[name]
                    print(f"✅ Found Feature Variable: '{name}'")
                    break

            input_A = None
            for name in ['H', 'A', 'adj', 'graph', 'adjacency', 'A_mat']:
                if name in locals():
                    input_A = locals()[name]
                    print(f"✅ Found Graph Variable: '{name}'")
                    break

            if input_X is None or input_A is None:
                print("\n" + "!"*50)
                print("❌ Error: 예측에 필요한 변수를 못 찾았습니다.")
                print(f"   - X Found? {input_X is not None}")
                print(f"   - A Found? {input_A is not None}")
                print("🔍 현재 사용 가능한 변수 목록:")
                keys = [k for k in locals().keys() if not k.startswith('__')]
                print(keys)
                print("!"*50 + "\n")
                exit()

            print("[Test Mode] Predicting (using X, A, test_indices)...")
            # predict 함수 호출
            preds, _ = model_instance.predict(input_X, input_A, target_idx)

            if target_label is not None:
                acc = accuracy_score(target_label, preds)
                print("="*40)
    ...
    if args.feature_report:
        ...
        train_vocab = set([term for term, count in Counter(np.nonzero(X[train_indices])[1]).items() if count >= 10])
