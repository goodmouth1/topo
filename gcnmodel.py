...
from os import path
import scipy as sp
print("DEBUG: Importing theano...")
import theano
print("DEBUG: Importing lasagne.utils...")
from lasagne.utils import floatX
print("DEBUG: Importing theano.tensor...")
import theano.tensor as T
print("DEBUG: Importing lasagne...")
import lasagne
print("DEBUG: Importing lasagne.regularization...")
from lasagne.regularization import regularize_layer_params_weighted, l2, l1
from lasagne.regularization import regularize_layer_params
print("DEBUG: Importing theano.sparse...")
import theano.sparse as S
print("DEBUG: Importing lasagne.layers...")
from lasagne.layers import DenseLayer, DropoutLayer
print("DEBUG: gcnmodel imports finished.")
...
def selu(x):
    ...
    return scale * T.where(x > 0.0, x, alpha * (T.exp(x) - 1.0))

def residual_dense(incoming, nonlinearity=selu):
    num_inputs = int(np.prod(incoming.output_shape[1:]))
...
class GraphConv():
    ...
    def fit(self, X, H, Y, train_indices, val_indices, n_epochs=10000, batch_size=1000, max_down=10, pseudolikelihood_thresh=0.2, verbose=True, seed=77):
        print("DEBUG: gcnmodel.fit called")
        sys.stdout.flush()
        np.random.seed(seed)
        ...
        y_dev = Y[val_indices]
        print(f"DEBUG: Starting training loop for {n_epochs} epochs...")
        sys.stdout.flush()
        for n in range(n_epochs):
            # if n % 10 == 0:
            print(f"DEBUG: Epoch {n} start")
            sys.stdout.flush()
            l_train, acc_train, l_val, acc_val, all_probs = self.f_train(X_train, y_train, y_dev, H, train_indices, val_indices)
            # if n % 10 == 0:
            print(f"Epoch {n}: Train Loss {l_train:.4f}, Train Acc {acc_train*100:.2f}%, Val Loss {l_val:.4f}, Val Acc {acc_val*100:.2f}%")
            sys.stdout.flush()
