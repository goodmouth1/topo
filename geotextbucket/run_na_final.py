#!/usr/bin/env python
"""
Twitter-US 미완료 실험 2개 실행 (na_bucket1000, na_bucket1500).
bucket300은 2048 클래스로 학습 불가 → 제외.

사용법:
  nohup /home/wang/miniconda3/envs/thesis_gpu/bin/python -u run_na_final.py > na_final.log 2>&1 &
  tail -f na_final.log
"""
...
def run_experiment(name, bucket, hidden, dropout, reg, cel, mindf):
    ...
    print(f"   GCN: Mean={gcn_metrics['mean_km']}km, Median={gcn_metrics['median_km']}km, "
    ...
    try:
        ...
        if 'gravity_acc' in gravity_metrics:
            print(f"   Gravity: {gravity_metrics['gravity_acc']}% "
                  f"({gravity_metrics['gravity_correct']}/{gravity_metrics['gravity_total']}), "
                  f"매칭 {gravity_metrics['matched_users']}명")
        else:
            print(f"   Gravity 결과 파싱 실패")
...
if __name__ == '__main__':
    print("=" * 60)
    print("Twitter-US 미완료 실험 (bucket1000, bucket1500)")
    print("=" * 60)
    ...
    for exp in EXPERIMENTS:
        try:
            ...
            if row:
                ...
                print(f"   저장 완료 (총 {len(all_results)}개)")
    ...
    print(f"전체 결과 요약")
    print(f"{'='*90}")
    print(f"{'데이터셋':<12} {'이름':<22} {'bucket':>6} {'클래스':>6} {'Median':>8} {'Acc@161':>8} "
          f"{'Gravity':>8}")
    print("-" * 90)
    for r in all_results:
        grav = f"{r.get('gravity_acc', 'N/A')}%"
        print(f"{r['dataset']:<12} {r['name']:<22} {r['bucket']:>6} "
              f"{'':>6} {r['median_km']:>7}km {r['acc_161']:>7}% {grav:>8}")
